...
def start_cfm_jobs(pos=True):
    ...
    for compound in db.compounds.find(search, {"SMILES": 1}):
        out_file.write("%s %s\n" % (compound['_id'], compound['SMILES']))
        i += 1
...
def load_cfm_results(result_dir, db_list, pos=True, cleanup=False):
    for spec_file in os.listdir(result_dir):
        if (spec_file[0] != "C") or (spec_file[-4:] != ".log"):
            continue
        with open(os.path.join(result_dir, spec_file)) as infile:
            ...
            try:
                for db in db_list:
                    if pos:
                        db.compounds.update({"_id": spec_file[:-4]}, {"$set": {"Pos_CFM_spectra": {"10 V": data[0],
                                            "20 V": data[1], "40 V": data[2]}}})
                    else:
                        db.compounds.update({"_id": spec_file[:-4]}, {"$set": {"Neg_CFM_spectra": {"10 V": data[0],
                                            "20 V": data[1], "40 V": data[2]}}})
            except IndexError:
                print(spec_file)
        if cleanup:
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "calculate":
        ...
        job_template = sys.argv[5]
        start_cfm_jobs(pos=False)
